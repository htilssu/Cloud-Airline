...
import { Plane, MapPin, Calendar, Users, Clock, ArrowRight, Check } from 'lucide-react';
...
const FlightDetailPage: React.FC = () => {
  ...
  const addPassenger = () => {
    setPassengers([...passengers, { name: '', ticketTypeId: 1, selectedAddons: [] }]);
  };
  ...
  const toggleAddon = (passengerIndex: number, addonId: number) => {
    ...
    if (selectedAddons.includes(addonId)) {
      updated[passengerIndex].selectedAddons = selectedAddons.filter(id => id !== addonId);
    } else {
  ...
  const calculateTotalPrice = () => {
    return passengers.reduce((total, passenger) => {
      const ticketType = ticketTypes.find(tt => tt.id === passenger.ticketTypeId);
      const ticketPrice = ticketType?.calculatedPrice || 0;
      
      const addonsPrice = passenger.selectedAddons.reduce((addonTotal, addonId) => {
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.detail || 'Có lỗi xảy ra khi đặt vé. Vui lòng thử lại.';
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-200 flex items-center justify-center">
        <div className="text-center text-blue-700">Đang tải thông tin chuyến bay...</div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-200 flex items-center justify-center">
        <div className="text-center">
          <div className="text-red-500 mb-4">{error || 'Không tìm thấy chuyến bay'}</div>
          <Button onClick={() => navigate('/flights')}>Quay lại danh sách chuyến bay</Button>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h1 className="text-2xl font-bold text-blue-900">Chi tiết chuyến bay {flight.flightNumber}</h1>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <div className="text-center">
              ...
              <div className="font-semibold text-blue-600 mt-2">{flight.departureAirport?.name}</div>
            ...
            <div className="text-center">
              ...
              <div className="font-semibold text-pink-600 mt-2">{flight.arrivalAirport?.name}</div>
        ...
        {/* Quick Overview */}
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
          <h2 className="text-xl font-bold text-blue-900 mb-4">Tùy chọn có sẵn</h2>
          ...
          <div className="grid md:grid-cols-2 gap-6">
                ...
                {ticketTypes.map((ticketType) => (
                  <div key={ticketType.id} className="flex justify-between items-center text-sm">
                    ...
                    <span className="text-blue-600 font-semibold">
                      {(ticketType.calculatedPrice || 0).toLocaleString('vi-VN')} ₫
                    </span>
                  </div>
                ))}
              </div>
            ...
            {/* Services Overview */}
            <div>
              <h3 className="font-semibold text-gray-700 mb-3">Dịch vụ bổ sung</h3>
              <div className="space-y-2">
          ...
          <div className="mt-4 p-4 bg-blue-50 rounded-lg">
            <p className="text-sm text-blue-700">
              💡 <strong>Hướng dẫn:</strong> Chọn loại vé và dịch vụ bổ sung cho từng hành khách bên dưới. 
              Mỗi hành khách có thể chọn loại vé và dịch vụ khác nhau.
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
          <div className="flex justify-between items-center mb-8">
            <h2 className="text-xl font-bold text-blue-900">Thông tin hành khách</h2>
            <Button onClick={addPassenger} variant="outline" className="bg-blue-50 hover:bg-blue-100">
              + Thêm hành khách
            </Button>
          </div>
            ...
            <div key={index} className="bg-gray-50 border rounded-xl p-6 mb-6">
              <div className="flex justify-between items-center mb-6">
                <div className="flex items-center space-x-3">
                  ...
                  <h3 className="font-semibold text-xl text-blue-900">Hành khách {index + 1}</h3>
                </div>
                {passengers.length > 1 && (
                  <Button 
                    variant="outline" 
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-2 gap-4">
                  {ticketTypes.map((ticketType) => (
                    <div 
                      ...
                      onClick={() => updatePassenger(index, 'ticketTypeId', ticketType.id)}
                    >
                      <div className="flex items-start justify-between mb-2">
                        <h4 className="font-semibold text-base text-gray-800">{ticketType.name}</h4>
                        {passenger.ticketTypeId === ticketType.id && (
                      ...
                      <p className="text-sm text-gray-600 mb-3 line-clamp-2">{ticketType.description}</p>
              ...
              <div className="bg-white rounded-lg p-4">
                ...
                <div className="grid md:grid-cols-1 lg:grid-cols-2 gap-3 max-h-60 overflow-y-auto">
                  {addonOptions.map((addon) => (
                    <label key={addon.id} className={`flex items-start p-3 rounded-lg border-2 cursor-pointer transition-all hover:shadow-md ${
                      ...
                      <div className="flex items-start space-x-3 w-full">
                        <div className="flex-shrink-0 mt-0.5">
                          <Checkbox
                            checked={passenger.selectedAddons.includes(addon.id)}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-gray-600 mb-2">{addon.description}</div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
          ...
          <div className="space-y-2 mb-6">
            {passengers.map((passenger, index) => {
              const ticketType = ticketTypes.find(tt => tt.id === passenger.ticketTypeId);
              const addonsPrice = passenger.selectedAddons.reduce((total, addonId) => {
                const addon = addonOptions.find(a => a.id === addonId);
              ...
              return (
                <div key={index} className="flex justify-between text-sm">
                  <span>
                    {passenger.name || `Hành khách ${index + 1}`} - {ticketType?.name}
                    {addonsPrice > 0 && ` + Dịch vụ`}
                  </span>
                  <span>
                    {((ticketType?.calculatedPrice || 0) + addonsPrice).toLocaleString('vi-VN')} ₫
                  </span>
                </div>
              );
            })}
          ...
          <p className="text-xs text-gray-500 mt-3 text-center">
            Sau khi đặt vé, bạn có 30 phút để thanh toán. Vé sẽ bị hủy tự động nếu không thanh toán trong thời gian này.
          </p>
        </div>
      </div>
    </div>
